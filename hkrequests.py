...
class HKRequests:
    ...
    def __init__(self, _sdate: str, _edate: str):
        ...
        self.suggestion_label = ['BUY', 'HOLD', 'NR', 'OUTPERFORM', 'REDUCE', 'STRONGBUY',
                                 'SUSPENDED', 'TRADINGBUY', 'UNDERPERFORM']
        self.suggestion_correction = [('-', 'NR'), ('NOTRATED', 'NR'), ('NA', 'NR'), ('N/A', 'NR'),
                                      ('중립', 'HOLD'), ('매수', 'BUY'), ('MARKETPERFORM', 'HOLD'),
                                      ('NEUTRAL', 'HOLD'), ('적극매수', 'STRONGBUY'), ('투자의견없음', 'NR')]
        ...
        assert self.end_page >= 1, "end_page가 자연수가 아님"
            ...
            try:
                with requests.get(url=f'{self.url}/apps.analysis/analysis.list', headers=self.headers, params=params) as req:
                    ...
                    for tag in html.select('tbody>tr'):
                        ...
                        in_raw_code: Match = company_code_compiler.search(raw_code.group()) if raw_code is not None else None

                        if in_raw_code is None:
                            continue
                        else:
                            company_code: str = in_raw_code.group()

                        company_name: str = title[:raw_code.start()].strip() if raw_code is not None else ''
                        company_name = re.sub(pattern=r'[\[\(]([가-힣\w\s])+[\]\)]', repl='', string=company_name)
                        ...
                        if (suggestion := tag.select('td')[3].get_text().strip().upper().replace(' ', '')) in self.suggestion_label:
                            pass
                        else:
                            for raw, cor in self.suggestion_correction:
                                if suggestion == raw:
                                    suggestion = cor
